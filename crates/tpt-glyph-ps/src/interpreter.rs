...
//
// TPT Glyph — tpt-glyph-ps / interpreter
//
...
// The dispatch table is derived from the tpt-glyph-kg knowledge graph catalog
...
use tpt_glyph_core::geometry::{Path, Point, Subpath, Transform};
use tpt_glyph_core::graphics_state::{GraphicsState, LineCap, LineJoin, RgbColor};
use tpt_glyph_core::render::RenderTree;
use tpt_glyph_kg::catalog::catalog;
use tpt_glyph_kg::OperatorCategory;
...
/// Build the operator dispatch table from the tpt-glyph-kg catalog. This ties the
...
mod tests {
    use super::*;
    use tpt_glyph_core::render::DrawCommand;
    ...
    fn setlinecap_setlinejoin_update_state() {
        ...
        assert_eq!(
            it.state().line_cap,
            tpt_glyph_core::graphics_state::LineCap::Round
        );
        assert_eq!(
            it.state().line_join,
            tpt_glyph_core::graphics_state::LineJoin::Bevel
        );
    }
