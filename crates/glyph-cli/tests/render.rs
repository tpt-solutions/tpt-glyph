...
//
// TPT Glyph — glyph-cli integration tests
//
// Drives the library crates (glyph-ps, glyph-pdf) the same way the CLI does and
...
use glyph_core::render::{DebugRasterizer, Rasterizer};
use glyph_pdf::PdfDocument;
use glyph_ps::Interpreter;
use std::path::Path;
...
fn cli_renders_pdf_fixture() {
    ...
    let canvas = glyph_pdf::render_page(&doc, 0, GraphicsState::new()).expect("render page");
