...
//
// TPT Glyph — glyph-pdf / content
//
// Maps PDF content-stream operators (`pdf::content::Op`) onto the shared
// glyph-core rendering pipeline. The interpreter maintains an immutable
// `GraphicsState` (reused from glyph-core), a per-page `TextState`, the
// in-progress `Path`, and the accumulated `RenderTree`.

use crate::error::{PdfError, Result};
use crate::fonts::decode_pdf_string;
use crate::xobject::decode_xobject;
use glyph_core::geometry::{CubicBezier, Path, Point, Subpath, Transform};
use glyph_core::graphics_state::{GraphicsState, LineCap, LineJoin, RgbColor};
use glyph_core::render::{DrawCommand, RenderTree};
use pdf::content::{
