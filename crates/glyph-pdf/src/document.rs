...
use glyph_core::document::Page;
use glyph_core::graphics_state::GraphicsState;
use glyph_core::render::{DebugRasterizer, Rasterizer, RenderTree};
...
impl PdfPageInfo {
    ...
    pub fn render(&self, state: GraphicsState, resolver: &impl Resolve) -> RenderTree {
        ...
        let mut path = glyph_core::geometry::Path::new();
        render_ops(
            &self.ops,
            &self.resources,
            resolver,
...
pub fn render_document(doc: &PdfDocument) -> Result<glyph_core::document::Document> {
    if doc.page_count() == 0 {
    ...
    let pages: Vec<Page> = pages
        ...
        .collect();
    Ok(glyph_core::document::Document { pages })
}
