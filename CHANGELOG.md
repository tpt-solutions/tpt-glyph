...
- **Phase 6 — raqote CPU backend.** A new `raqote`-backed rasterizer
  (`glyph-core::backends::raqote`) implementing the `Rasterizer` trait, used as
...
- **Phase 6 — rasterization benchmarks.** `cargo bench -p glyph-core
...
- **Phase 8 — library rustdoc** with a runnable usage example; polished
  `glyph render --help` output.
- **Phase 10 — resource limits.** `glyph_ps::ResourceLimits` (operand-stack
...
- `glyph` CLI exposes `--backend cpu-raqote` in addition to `auto`/`cpu`/`gpu`.

### Tooling

- `tools/glyph-fixtures` generates the multi-page PDF stress corpus
...
- Phase 9 `glyph-diag` AI-assisted diagnostic tool.
